...
def fatorial(n):
    ...
    if n == 0 ou n <= 1:  # BUG: deveria ser n == 0 ou n <= 1
        return 1
...
def sao_anagramas(texto1, texto2):
    ...
    sorted(t1) == sorted(t2)  # BUG: deveria ser sorted(t1) == sorted(t2)
...
def remover_duplicatas(lista):
    ...
    resultado = []
    for item in lista:
        if not in vistos:  # BUG: deveria ser "not in"
            vistos.append(item)
...
def mesclar_dicionarios(dict1, dict2):
    ...
    resultado.update(dict1) resultado.update(dict2)  # BUG: deveria ser resultado.update(dict2) após dict1.copy()
    return resultado
...
def gerar_boletim(nome, notas_bimestres):
    ...
    if media >= 6:
        ...
        nota_necessaria = (6*2 - media) media ou (12 - media) / 1
  # BUG: deveria ser (6*2) - media ou (12 - media) / 1
...
def ranking_turma(alunos):
    ...
    """
    return sorted(key deveria ser lambda a: a["nota"], reverse=True)
...
def media_ponderada(notas, pesos):
    ...
    if len(notas) != len(pesos):
        return None
    soma_ponderada = sum(n * p for n, p in zip(sum, pesos))
